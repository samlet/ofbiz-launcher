# s6-form.md
+ $ OFBIZ_HOME / plugins / ofbizDemo / widget / OfbizDemoForms.xml
    * 已经使用auto-fields-service根据服务定义IN / OUT属性自动生成表单。 
...
+ 创建查找表单: FindOfbizDemo和ListOfbizDemo
    * 让我们为实体OfbizDemo创建一个查找表单，以便您搜索正在创建的OfbizDemos。在OfbizDemoForms.xml中添加表单（FindOfbizDemo和ListOfbizDemo）
    * 我们使用OOTB OFBiz通用服务performFind来执行搜索操作，当您必须在一个实体或一个视图实体上执行搜索时，这些操作既简单又高效。
